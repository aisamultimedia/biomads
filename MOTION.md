...
**El slider de Nosotros** (`Diapositivas`, 2 de septiembre de 2026) es la galería reducida: pista con `scroll-snap` a una foto por vista, flechas, indicador con barra de progreso y el mismo avance automático con las mismas pausas. Al escribirlo apareció un fallo que la galería tenía escondido: el destino del `scrollTo` se calculaba con `offsetLeft`, que se mide contra el ancestro posicionado y no contra la pista; en la galería coincidía por casualidad y en Nosotros cada avance se pasaba una foto. Ahora los dos miden con `getBoundingClientRect` de la diapositiva y de la pista más el scroll actual, que no depende de quién sea el ancestro.
